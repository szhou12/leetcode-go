...
* 个人总结:
    ...
    * 但是要注意！如果函数里要对slice进行 append() 操作，则必须传引用 `fcn(*nums)`， 直接传slice `fcn(nums)` 没用。 因为，`append()` 和 `copy()` 一样会深拷贝，即生成新的一个底层数组，所以，指针地址也会改变。也就是说，function内部一旦发生 `append()`，它赋值给的slice就已经不再是输入时候的slice了。
